...
发版才更新，粒度粗（按家族），分不清 `mimo-v2.5-pro`（纯文本）与 `mimo-v2.5`
（全模态）——导致给 `mimo-v2.5-pro` 发图触发小米 HTTP 404（[XiaomiMiMo/MiMo#62](https://github.com/XiaomiMiMo/MiMo/issues/62)）。
...
| `overrides.json`（本仓库人工维护） | 厂商私有项强制覆盖（如小米 `forced_search` 联网；锁死 `mimo-v2.5-pro` 纯文本防上游波动） |

合并策略：同纯 id 多条 → 能力 **OR 并集**；`overrides.json` 优先级最高（强制覆盖/新增）。
...
> 否则 deepseek 等被误标 `openai` 后走 `/v1/responses` 触发 404。xAI（grok）在
> OtterPad App 侧 `wireProtocol` 升格走 Responses，数据层仍归 `openAICompatible`。
...
+ `overrides.json`，写出 `model_capabilities.json`。
...
上游覆盖不到的厂商私有项在此强制覆盖。`_` 前缀字段是说明（不进能力表）：

```json
{
  "mimo-v2.5-pro": {
    "imageInput": false, "tool": true, "reasoning": true, "webSearch": true,
    "providers": ["openAICompatible"],
    "_note": "小米 MiMo 旗舰（纯文本），图片输入触发 404；联网走 forced_search"
  }
}
```
